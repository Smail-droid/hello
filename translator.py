...
def generate_polite_response(text):
    ...
    try:
        ...
        data = {
            "model": "gpt-3.5-turbo",
            "messages": [
                {"role": "system", "content": "你是一个擅长增进感情和建立良好关系的助手。请根据用户输入的内容，生成一个简短但温暖的回复。回复要求：1. 控制在30字以内 2. 直接表达关心和支持，不要以'我理解'、'我明白'等开场 3. 使用温暖友善的语气 4. 不要使用任何表情符号 5. 不要出现'加油'、'祝你好运'等鼓励性口号 6. 保持自然，避免过于做作 7. 重点突出：给予真诚的赞美，表达关心和在意。请记住，简短但温暖的回复往往更有力量。"},
                {"role": "user", "content": text}
            ],
            "temperature": 0.7
        }
